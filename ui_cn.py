...
def run_img2puzzles_llm(_key, _prompt):
    _key = "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"
    if _key and _prompt:
...
with gr.Blocks(title="从封面图片生成益智题目集") as demo:
    ...
    with gr.Tab(label = ""):
        with gr.Row():
            openai_api_key = gr.Textbox(label="OpenAI API Key", placeholder="sk-**********", lines=1, visible=False)
        with gr.Row():
...
if __name__ == "__main__":
    demo.queue(concurrency_count=1).launch(server_name="0.0.0.0", server_port=7860, share=False, favicon_path="./favicon_puzzle.jpg")
